...
class SAKT(Module):
    def __init__(
        ...
        num_heads: int,
        dropout: float = 0.2,
        pre_embeddings: dict[str, torch.Tensor] | None = None,
    ) -> None:
        ...
        Args:
            ...
            pre_embeddings (dict[str, torch.Tensor] | None, optional): The pre-embeddings for the model. Defaults to None.
        """
        super().__init__()
    ...
    def forward(
        ...
        Args:
            ...
            pre_embedding_names (list[str] | None): The names of the pre-embeddings to use.
                Defaults to None.
        ...
        if pre_embedding_names:
            for pre_embedding_name in self.pre_embedding_names:
                pre_embedding_map = getattr(self, pre_embedding_name)
                # Select the pre-embedding based on the question IDs
                pre_embedding = pre_embedding_map[question_ids[:, 1:]]
                x += pre_embedding
